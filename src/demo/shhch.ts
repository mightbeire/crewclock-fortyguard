...
export type ExceedanceWindow = {
  ...
  resultHash: string; version: string; qualifying?: boolean; tiles: ExceedanceTile[]; workfaceIds?: string[]; workface_id?: string; window_id?: string
}
...
const weightedTileHours = (workface: Polygon, tiles: ExceedanceTile[]) => { const totalArea = tiles.reduce((sum, tile) => sum + intersectionArea(workface, tile.polygon), 0); if (totalArea <= 0) return null; return tiles.reduce((sum, tile) => sum + intersectionArea(workface, tile.polygon) * tile.valueHours, 0) / totalArea }
...
export const calculateScheduledHighHeatCrewHours = (schedule: Record<string, string>, tasks: ReadonlyArray<{ id: string; durationMinutes: number; crewId: string; zoneId: string; environment: string; fixed: boolean }>, crews: ReadonlyArray<{ id: string; headcount: number }>, workfaces: ReadonlyArray<{ id: string; polygon: Polygon }>, windows: ReadonlyArray<ExceedanceWindow>, trigger: ProjectThermalTrigger): ShhchResult => {
  ...
  tasks.filter(task => task.environment !== 'shaded-support').forEach(task => {
    ...
    const scopedWindows = windows.filter(window => !window.workfaceIds?.length || window.workfaceIds.includes(task.zoneId) || window.workface_id === task.zoneId)
    if (!scopedWindows.length) { errors.push(`uncovered_task_interval:${task.id}`); return }
    const taskStart = minutes(start); const taskEnd = taskStart + task.durationMinutes; const boundaries = new Set([taskStart, taskEnd])
    scopedWindows.forEach(window => { boundaries.add(Math.max(taskStart, minutes(window.start))); boundaries.add(Math.min(taskEnd, minutes(window.end))) })
    const points = [...boundaries].filter(point => point >= taskStart && point <= taskEnd).sort((a, b) => a - b)
    const provenance = new Set<string>(); let exceedance = 0; let invalidWindow = false; let missingSpatialCoverage = false
    scopedWindows.forEach(window => { if (window.analyticType !== 'exceedance' || !['hour', 'hours'].includes(window.units) || window.status !== 'VALID' || !window.provenance || !window.aoi || !window.date || !window.timezone || !window.analyticSource || !window.resultHash || !window.version || window.projectThermalTrigger.thresholdC !== trigger.thresholdC || window.projectThermalTrigger.quantity !== trigger.quantity || window.projectThermalTrigger.thresholdUnits !== trigger.thresholdUnits || window.projectThermalTrigger.direction !== trigger.direction) invalidWindow = true })
    if (invalidWindow) { errors.push(`invalid_exceedance_window:${task.id}`); return }
    if (points.slice(0, -1).some((left, index) => points[index + 1] > left && !scopedWindows.some(window => minutes(window.start) <= left && points[index + 1] <= minutes(window.end)))) { errors.push(`uncovered_task_interval:${task.id}`); return }
    points.slice(0, -1).forEach((left, index) => { const right = points[index + 1]; const covering = scopedWindows.filter(window => minutes(window.start) <= left && right <= minutes(window.end)); if (!covering.length) return; const rates = covering.flatMap(window => { const weighted = weightedTileHours(face.polygon, window.tiles); if (weighted === null) { missingSpatialCoverage = true; return []; } return [{ rate: window.qualifying === false ? 0 : Math.max(0, Math.min(1, weighted / ((minutes(window.end) - minutes(window.start)) / 60))), provenance: window.provenance }] }); const selected = rates.reduce((best, item) => item.rate > best.rate ? item : best, { rate: 0, provenance: '' }); exceedance += (right - left) / 60 * selected.rate; rates.forEach(item => provenance.add(item.provenance)) })
    if (missingSpatialCoverage) { errors.push(`workface_does_not_overlap_exceedance_tiles:${task.id}`); return }
