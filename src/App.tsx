...
function WorkflowRail({ events }: { events: RuntimeSession['events'] }) {
  return <section className="workflow-rail" aria-label="CrewClock review progress">
    <div className="section-kicker"><span>Review path</span><span>{events.length ? `${events.length} runtime events` : 'Ready when you are'}</span></div>
...
export default function App() {
  ...
  return <main className="app-shell" data-state={verified ? 'verified' : revealed ? 'proposed' : investigating ? 'investigating' : 'initial'}>
    <header className="topbar"><div className="brand"><span className="brand-mark"><Clock3 size={16}/></span><span className="brand-wordmark">CREWCLOCK<small>FIELD PLANNING</small></span></div><div className="top-context"><span className="live-dot"/>PHOENIX INDUSTRIAL PROJECT <em>·</em> SAMPLE PROJECT</div><div className="top-actions"><button onClick={() => setDrawer('evidence')}><Database size={14}/>Evidence</button><button onClick={() => setDrawer('audit')}><ClipboardCheck size={14}/>Audit</button><button className="reset-control" onClick={reset}><RotateCcw size={13}/>Reset sample project</button></div></header>
    ...
    <section className="scene-intro"><div><span className="eyebrow">SHIFT WATCHPOINT <i/> THERMAL / OPERATIONAL</span><h2 className="display-type">{run.investigation.investigatedTaskIds.length} outdoor tasks need attention</h2><p>{attentionText}</p></div><div className="intro-legend"><span><i className="orange-dot"/>attention</span><span><i className="lime-dot"/>CrewClock action</span><span><LockKeyhole size={12}/>fixed</span></div></section>
